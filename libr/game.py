"""
Ce module comprend la gestion des évènements (classe Game), la création de la map (classe City), ainsi que la définition des briques (classe Brick)
"""

import pygame
from pygame.locals import *

from libr.sprites import *
    ...
    def run(self):
        ...
        while True:
            ...
            if self.vehicle.alive == True:
                ...
                self.turret.update(self.vehicle.rect.center, m_x, m_y, self.camera.topleft, self.keys, self.bullets,
                                   self.vehicle.angle)
            ...
            self.killshow = self.font4.render('Kills ' + str(self.enemies - self.score) + ' of ' + str(self.enemies),
                                              False, (255, 255, 255))
            ...
            pygame.draw.ellipse(self.screen, (255, ((self.vehicle.health * 255) / 25), 0),
                                (92 + (100 * (float(self.vehicle.health) / float(25))), 20, 10, 13))
            pygame.draw.ellipse(self.screen, (0, 230, 0), (296, 20, 10, 13))
            pygame.draw.ellipse(self.screen, (0, 230, 0),
                                (298 + (100 * (float(self.turret.timer) / float(40))), 20, 10, 13))

            self.screen.fill((255, ((self.vehicle.health * 255) / 25), 0),
                             (96, 20, (100 * (float(self.vehicle.health) / float(25))), 13))
        ...
        while self.y < self.height:
            ...
            if color in collidable:
                ...
                if color == collidable[0]:
                    self.bricks.add(
                        Brick((self.x * 50, self.y * 50), self.brick, top, bottom, right, left, destroyable, bricks_des,
                              bricks_non))
                if color == collidable[1]:
                    self.bricks.add(
                        Brick((self.x * 50, self.y * 50), self.plate, top, bottom, right, left, False, bricks_des,
                              bricks_non))
            if color == self.spawn:
